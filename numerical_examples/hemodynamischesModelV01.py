...
x=np.zeros((12,n))  
x_0=np.array([1,1,1,2,2,2,3,3,3,4,4,4,5,5,5]) #x,s,f,v,q je drei Zeileneinträge
x_0=np.transpose(x_0)       #transponiert
x[:,0]=x_0                  #Trägt Anfangsbdg in die erste Spalte ein
...
T=2.1    #Endzeit
t0=2    #Anfangszeit
n=256   #Intervallanzahl
dt=(T-t0)/n              #Zeitschrittlaenge
t = np.arange(t0,T,dt)      #Zeitarray


A=np.matrix("-0.4, 0, 0; 0, -0.1, 0; 0, 0, -0.3")    #Matrix A (3x3) negative Einträge auf der Hauptdiagonalen
...
const=C*u #C*u soll zeitabhänigig aber vorgegeben sein
...
def f(x,teta):       #Gibt die Zeitableitung x_dot wider
    ...
    z=np.vsplit(x,(0,N-1))[1]         #Die zeitabhängigen Variablen werden aus dem Gesamtvektor herausgeschnitten
    s=np.vsplit(x,(N,2*N-1))[1]
    f=np.vsplit(x,(2*N,3*N-1))[1]
    v=np.vsplit(x,(3*N,4*N-1))[1]
    q=np.vsplit(x,(4*N,5*N-1))[1]
