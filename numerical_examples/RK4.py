...
import numpy as np
import math
def RK4_method(f,A,B,C,u,z_0,dt,t0,T):  
    #Input: Funktion, Matrizen A,B und C,Anregung, Anfangswert(array), Zeitschrittweite,Anfangszeit, Endzeit
    ...
    for i in range(0,int(math.ceil((T-t0)/dt-1)),1):
        ...
        k_1 = f(z_aktuell,A,B,C,u)                  #Mit dem aktuellen Outputvektor werden die k's ermittelt
        k_2 = f(z_aktuell+ 0.5*dt*k_1,A,B,C,u)
        k_3 = f(z_aktuell + 0.5*dt*k_2,A,B,C,u)
        k_4 = f(z_aktuell + dt*k_3,A,B,C,u)
